...
bool kangaroo::start( )
{
	...
	*/
	if( !ack_sub )
	ack_sub = nh.subscribe( "ack_cmd", 1, &kangaroo::AckCB, this );
...
void kangaroo::AckCB( const ackermann_msgs::AckermannDriveStampedPtr &msg ){
	double channel_1_position;	//Steer Angle
	double channel_2_position;	//Target Speed

	ackermann_msgs::AckermannDrive stats = msg->drive;
